...
namespace NMib::NCloud
{
		...
		for (auto &AppManager : m_AppManagerInfos)
		{
			if (m_Options & EOption_LaunchTestAppInApp)
			{
				if (_VersionManagerApplication == "TestApp")
					m_InProcessLaunchScopes.f_Insert(fg_AppManager_RegisterInProcessFactory(AppManager.m_RootDirectory / "App" / _Name / "TestApp", &fg_ConstructApp_TestApp));
			}
	...
	void CAppManagerTestHelper::f_CheckCloudManager(mint _Secuence)
	{
		DMibTestPath("CloudManager{}"_f << _Secuence);
	...
	void CAppManagerTestHelper::f_Setup(mint _nAppManagers)
	{
		...
		{
			auto HostID = AppManager->f_GetDistributedActorData()->f_GetHostInfo().m_RealHostID;
