...
namespace NCodeSigningManagerTests
{
		...
		)
	{
		...
		Launch.m_Params.m_fOnOutput = [LaunchActor, pResult, StdInBuffer = CStr(), nStdInIndex = umint(0), _StdInLines](EProcessLaunchOutputType _OutputType, CStr const &_Output) mutable
			{
		...
		void f_DoTests()
		{
			DMibTestSuite("General") -> TCFuture<void>
			{
				...
				// Provide password to KeyManager
				{
					...
					CProcessLaunchActor::CSimpleLaunch LaunchParams{KeyManagerDirectory + "/KeyManager", {"--provide-password"}, KeyManagerDirectory, CProcessLaunchActor::ESimpleLaunchFlag_None};
				...
				// Setup MalterlibCloud trust to connect to CodeSigningManager
				{
					...
					auto CloudClientHostIDResult = co_await CProcessLaunchActor::fs_LaunchSimple
						(
							CProcessLaunchActor::CSimpleLaunch{CloudClientExe + CFile::mc_ExecutableExtension, fGetParams({"--trust-host-id"}), CloudClientDirectory, CProcessLaunchActor::ESimpleLaunchFlag_GenerateExceptionOnNonZeroExitCode}
						)
					;
