...
namespace NMib::NCloud
{
	...
	TCFuture<void> CBackupManagerClient::CInternal::CDistributedAppInterfaceBackupImplementation::f_AppendManifest(CManifestConfig _Config)
	{
		...
		for (auto &Destination : _Config.m_IncludeWildcards)
		{
			...
			if (pOldDestination)
			{
				if (*pOldDestination != Destination)
					co_return DMibErrorInstance("You cannot change the destination for a wildcard");
			}
			else
		...
		auto Result = co_await Internal.f_SubscribeChanges().f_Wrap();

		if (pThis->f_IsDestroyed())
			co_return DMibErrorInstance("Destroyed");

		if (!Result)
			Internal.f_ReportBackupError("Failed to subscribe to file notifications when appending manifest: {}"_f << Result.f_GetExceptionStr(), false);

		auto BlockingActorCheckout = fg_BlockingActor();
		auto Manifest = co_await
			(
				g_Dispatch(BlockingActorCheckout) / [NewConfig = fg_Move(NewConfig), pDestroyed = Internal.m_pDestroyed]()
		...
		if (!Manifest)
		{
			Internal.f_ReportBackupError("Failed to get manifest when appending: {}"_f << Manifest.f_GetExceptionStr(), true);
			co_return Manifest.f_GetException();
		}

		auto &NewManifest = fg_Get<0>(*Manifest);
		auto &NewManifestFileIDs = fg_Get<1>(*Manifest);

		auto &AppendStates = fg_Get<2>(*Manifest);
