...
namespace NMib::NCloud::NBackupManager
{
		...
		-> TCContinuation<TCDistributedActorInterfaceWithID<CDirectorySyncClient>>
	{
		...
		while (!bFullAccess)
		{
			...
			if (pThis->mp_Permissions.f_HostHasPermission(CallingHostID, fg_Format("Backup/Read/{}", _BackupSource)))
				break;
		...
		BackupPathContinuation.f_Dispatch() > Continuation % Auditor / [=, Subscription = fg_Move(_Subscription)](CStr const &_BackupPath) mutable
			{
