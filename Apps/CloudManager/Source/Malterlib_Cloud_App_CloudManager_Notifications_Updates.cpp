...
namespace NMib::NCloud::NCloudManager
{
	...
	TCFuture<void> CUpdateNotifications::fp_SendDeferredNotifications()
	{
		...
		for (auto &Update : DeferredUpdates.m_OrderedUpdates)
			co_await fg_CallSafe(*this, &CUpdateNotifications::fp_ReportApplicationUpdateToSlack, Update.m_AppManagerID, Update.m_LastUpdateState, Update.m_LastNotification);
	...
	TCFuture<void> CUpdateNotifications::f_ProcessApplicationUpdateNotification(CStr _AppManagerID, CAppManagerInterface::CUpdateNotification _Notification)
	{
		...
		if (!_Notification.m_UpdateID)
			co_await fg_CallSafe(*this, &CUpdateNotifications::fp_ReportApplicationUpdateToSlack, _AppManagerID, UpdateState, _Notification);
		else
		{
			...
			Update.m_LastNotification = fg_Move(_Notification);
		...
		)
	{
		...
		if (_Notification.m_Stage == CAppManagerInterface::EUpdateStage_Finished)
		{
			SlackAttachment.m_Title = "Update successful";
			SlackAttachment.m_Color = CSlackActor::EPredefinedColor_Good;
			bFinished = true;
		}
		else if (_Notification.m_Stage == CAppManagerInterface::EUpdateStage_Failed)
		{
			...
			SlackAttachment.m_Title = "Updating ({})"_f << CAppManagerInterface::fs_UpdateStageToStr(_Notification.m_Stage);

		if (_Notification.m_Message)
			fg_AddStrSep(SlackAttachment.m_Text, "```{}```"_f << CSlackActor::CMessage::fs_EscapeString(_Notification.m_Message), "\n\n");
		...
		for (auto &StageState : _UpdateState.m_Stages)
		{
			...
			if
				(
					...
					&& ((PreviousStageTime > mp_UpdateStageNotificationThreshold) || (!bFinished && Stage == _Notification.m_Stage))
				)
			{
		...
		if (!bFinished)
			TimingString += "{cc,sj30,a-}...\n"_f << CAppManagerInterface::fs_UpdateStageToStr(_Notification.m_Stage);
			...
			(
				{
					...
					{
						"Application"
						, "`{}`"_f << CSlackActor::CMessage::fs_EscapeString(_Notification.m_Application)
						, true
					}
					,
					{
					...
					,
					{
						"Version"
						, "`{}/{}.{}.{}`\n{}"_f
						<< CSlackActor::CMessage::fs_EscapeString(_Notification.m_VersionID.m_VersionID.m_Branch)
						<< _Notification.m_VersionID.m_VersionID.m_Major
						<< _Notification.m_VersionID.m_VersionID.m_Minor
						<< _Notification.m_VersionID.m_VersionID.m_Revision
						<< CSlackActor::CMessage::fs_EscapeString(_Notification.m_VersionID.m_Platform)
						, true
					}
					,
					{
						"Total Duration"
						, fg_SecondsDurationToHumanReadable(_Notification.m_UpdateTime)
						, true
					}
					,
					{
		...
		if (_Notification.m_StartUpdateTime.f_IsValid())
			SlackAttachment.m_FooterTimestamp = _Notification.m_StartUpdateTime + CTimeSpanConvert::fs_CreateSpanFromSeconds(_Notification.m_UpdateTime);
		...
		if (bFinished && _Notification.m_UpdateTime > mp_UpdateLongRunningThreshold)
		{
			CSlackActor::CMessage LongRunningMessage;
			LongRunningMessage.m_Text = "Long running update of `{}` on `{}` finished"_f
				<< CSlackActor::CMessage::fs_EscapeString(_Notification.m_Application)
				<< CSlackActor::CMessage::fs_EscapeString(AppManagerInfo.m_HostName)
			;
...

		{
			auto WriteResult = co_await mp_This.mp_DatabaseActor
				(
					&CDatabaseActor::f_WriteWithCompaction
					, g_ActorFunctorWeak / [this, Notification = _Notification, _AppManagerID, SlackTimestamps = fg_Move(SlackTimestamps)]
