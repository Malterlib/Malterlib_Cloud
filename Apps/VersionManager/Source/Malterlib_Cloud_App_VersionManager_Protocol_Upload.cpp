...
namespace NMib::NCloud::NVersionManager
{
		...
		-> TCContinuation<CVersionManager::CStartUploadVersion::CResult> 
	{
		...
		while (!bFullAccess)
		{
			if (mp_Permissions.f_HostHasPermission(_CallingHostInfo.f_GetRealHostID(), fg_Format("Version/Write/{}", _Params.m_Application)))
				break;
		...
		
		{
			...
			{
				Permissions[fg_Format("Application/Read/{}", _Params.m_Application)];
				Permissions[fg_Format("Application/Write/{}", _Params.m_Application)];
			}
		...
		Upload.m_FileTransferReceive(&CFileTransferReceive::f_ReceiveFiles, _Params.m_QueueSize, ReceiveFlags) 
			> [pCleanup, this, _CallingHostInfo, UploadID, Continuation, Desc = Upload.m_Desc, _Params, VersionInfo, VersionPath]
			(TCAsyncResult<CFileTransferContext> &&_FileTransferContext) mutable
			{
				...
				fg_Dispatch
					...
					(TCAsyncResult<CVersionManager::CStartUploadTransfer::CResult> &&_Result, TCAsyncResult<void> &&_InfoWriteResult)
					{
						bool bFailed = false;
						if (!_Result)
						{
				...
				Upload.m_FileTransferReceive(&CFileTransferReceive::f_GetResult) > [this, _CallingHostInfo, UploadID, Desc](TCAsyncResult<CFileTransferResult> &&_Result)
					{
						if (!_Result)
