...
namespace NMib::NCloud::NCloudAPIManager
{
	auto CCloudAPIManagerDaemonActor::CServer::CCloudAPIManagerImplementation::f_GetSwiftBaseURL(CGetSwiftBaseURL &&_Params) -> TCContinuation<CGetSwiftBaseURL::CResult>
	{
		...
		if (!pThis->mp_Permissions.f_HostHasAnyPermission(fg_GetCallingHostID(), "ObjectStorage/GetSwiftBaseURLAll", fg_Format("ObjectStorage/GetSwiftBaseURL/{}", _Params.m_CloudContext)))
			Auditor.f_AccessDenied("(Get Swift base URL)");
